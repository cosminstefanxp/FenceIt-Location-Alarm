...
/**
 * A location defined by the user. It can be of various types and should contain the defining details so that,
 * using the provided contextual information, it should be determined if the device is or is not inside the
 * fence/location.
 */
public interface AlarmLocation extends DatabaseReferenceClass {
	...
	public enum Status {
		...
		LEFT,
		/**
		 * There is not enough information to decide anything (usually the required context data is not
		 * available).
		 */
		UNKNOWN
	};
	 ...
	 * 
	 * @param info the context information which provides enough details to check if the device is inside the
	 *        fence/location
	 * @return the status
	 */
	...
	/**
	 * Gets the display name. It is either the location's name, if any, or a default text for anonymous
	 * locations.
	 * 
