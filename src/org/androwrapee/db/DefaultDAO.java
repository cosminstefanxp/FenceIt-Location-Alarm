...
public class DefaultDAO<T> {
	...
	public boolean update(T object, long rowId) {
		...
		try {
			...
			log.fatal("Error occured while parsing object for update: " + object + ". Error message: " + e.getMessage());
			e.printStackTrace();
			return false;
		}
	...
	protected ContentValues buildContentValues(T object, boolean setID) throws IllegalArgumentException,
		...
		for (Field field : mReflectionManager.getReferenceFields()) {
			...
			if (reference == null)
				continue; // don't add
			Method referenceGetMethod = reference.getClass().getDeclaredMethod("getId", (Class[]) null);
